...
import { resolveAsset } from "../core/paths.js";

export const PROJECT_MEDIA_LIMITS = Object.freeze({ images: 12, videos: 4, total: 16 });

const LABELS = {
    en: {
        gallery: "Project media",
        empty: "Showcase media pending.",
    ...
    es: {
        gallery: "Medios del proyecto",
        empty: "Medios del showcase pendientes.",
...
export function limitProjectMedia(source = []) {
    const counts = { image: 0, video: 0 };
    return source.reduce((items, item) => {
        if (!item?.src || !["image", "video"].includes(item.type)) return items;
        if (items.length >= PROJECT_MEDIA_LIMITS.total) return items;
        const limit = item.type === "image" ? PROJECT_MEDIA_LIMITS.images : PROJECT_MEDIA_LIMITS.videos;
...
export function createProjectMediaGallery(sourceMedia, { language, title }) {
    const labels = LABELS[language];
    const media = limitProjectMedia(sourceMedia);
    const root = createElement("section", {
        className: "project-media",
        attributes: { "aria-label": labels.gallery, tabindex: "0" }
    });
    const stage = createElement("div", { className: "project-media__stage" });
    const viewport = createElement("div", { className: "project-media__viewport" });
    const actions = createElement("div", { className: "project-media__actions" });
    const theaterButton = createControl("THEATER", labels.theater, "project-media__theater-button");
    const fullscreenButton = createControl("⛶", labels.fullscreen, "project-media__fullscreen-button");
    const carousel = createElement("div", { className: "project-media__carousel" });
    const previousButton = createControl("←", labels.previous, "project-media__previous");
    const nextButton = createControl("→", labels.next, "project-media__next");
    const strip = createElement("div", {
        className: "project-media__strip",
    ...
    const anchor = createElement("div", { className: "project-media__anchor", attributes: { "aria-hidden": "true" } });
    const theater = createElement("div", {
        className: "project-media-theater",
        attributes: { role: "dialog", "aria-modal": "true", "aria-label": labels.gallery }
    });
    const closeTheaterButton = createControl("×", labels.exitTheater, "project-media-theater__close");
    ...
    function openTheater() {
        ...
        document.body.classList.add("has-project-theater");
    ...
    async function closeTheater() {
        ...
        theater.remove();
        document.body.classList.remove("has-project-theater");
    ...
    function createPrimaryMedia(item, index) {
        ...
        const button = createElement("button", {
            className: "project-media__image-button",
            attributes: { type: "button", "aria-label": labels.openImage }
        });
        const image = createMediaImage(item.src, alt, "project-media__image");
        image.loading = "eager";
        button.append(image);
    ...
    media.forEach((item, index) => {
        ...
        const thumbnail = createElement("button", {
            className: `project-media__thumbnail project-media__thumbnail--${item.type}`,
            attributes: {
                type: "button",
        ...
        if (previewPath) thumbnail.append(createMediaImage(previewPath, "", "project-media__thumbnail-image"));
        if (item.type === "video") {
            thumbnail.append(createElement("span", {
                className: "project-media__video-badge",
                text: "▶",
    ...
    if (media.length) {
        ...
        viewport.append(createElement("p", { className: "project-media__empty", text: labels.empty }));
