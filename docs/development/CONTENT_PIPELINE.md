...
Importer version 11 extends the CV compiler with source-authored rich-text boundaries. Blank Word paragraphs are retained for CV structural analysis: within prose/list sequences they close the active group, contribute a `break` parity token, and mark the following semantic block with `data-authored-break-before="true"` without generating empty paragraphs or `<br><br>` spacing. Inline `w:rPr/w:b` and `w:rPr/w:i` formatting remains semantic `<strong>`/`<em>` output in Normal paragraphs and list items; adjacent runs with equivalent emphasis and hyperlink targets, including bold whitespace, are coalesced without changing source text. Heading levels continue to come only from resolved Word paragraph styles, and ES/EN parity compares list and authored-boundary topology rather than translated wording or emphasis offsets.
...
A trailing `Resources` Heading 2 in a Game DOCX is reserved shared technical metadata and is removed before the ES/EN split. Its `Youtube Videos:` subsection accepts `Link 1:` through `Link 4:` as direct `youtube.com/watch?v=...` or `youtu.be/...` video URLs; empty links are ignored and non-video/search/channel URLs are rejected. Accepted links compile to `youtubeVideos` objects containing `provider`, `videoId`, and the authored `sourceUrl`. Resources never participate in bilingual parity and never appear in generated prose.
...
Game editorial hierarchy is schema-driven and semantic. The title is `Heading1`; Game main sections such as `El Juego` / `The Game` and `Contribución` / `Contribution` are `Heading2`; genuine authored subsections are `Heading3`. The Game schema also defines exact normal-paragraph field labels for `Main Features` beneath overview and the known localized Contribution children. Each recognized field compiles to `<h3>` only beneath its declared parent and owns the following authored block. These explicit field rules do not inspect Word typography and do not promote arbitrary normal paragraphs.
...
Project and Game images/videos are public media assets referenced through the same ordered registry item contract and rendered by one shared engine. Each viewer may display at most 12 images and four videos, with 16 items total. For Games, the four video slots are shared by authored YouTube resources and local videos: YouTube appears first in authored Link order, local videos fill remaining slots, and images follow. Project files use `assets/projects/<project-id>/media/images/` and `media/videos/`; new Game files use `assets/games/<game-id>/media/images/` and `media/videos/`. Metadata identifies media type, source, optional video poster, localized labels or alternative text, ordering, formats, and accessibility requirements independently from page templates. Consumer CSS owns geometry: selected Game images use centered `contain` fitting within the stable `1112 / 628` stage, permitting dark letterboxing without cropping authored content; compact thumbnails retain `cover`. Legacy Project `prototype/videos/`, Game `gallery/`, and their registered paths remain preserved without creating separate viewer implementations.
